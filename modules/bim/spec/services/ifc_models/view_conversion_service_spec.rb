...
RSpec.describe Bim::IfcModels::ViewConverterService do
  ...
  before do
    ...
    described_class.instance_variable_set(:@available_commands_cache_key, nil)
  end
  ...
  describe "#available_commands" do
    ...
    context "with bundled IfcConvert path" do
      before do
        allow(subject).to receive(:bundled_ifc_converter_path).and_return("/opt/ifc/IfcConvert")
        allow(File).to receive(:file?).with("/opt/ifc/IfcConvert").and_return(true)
        allow(File).to receive(:executable?).with("/opt/ifc/IfcConvert").and_return(true)

        allow(Open3)
          .to(receive(:capture2e))
          .with("which", any_args)
          .and_wrap_original do |_, *args|
          result = OpenStruct.new(exitstatus: args[1] == "IfcConvert" ? 1 : 0)
          ["irrelevant output", result]
        end
      end

      it "uses the bundled binary for IfcConvert availability" do
        expect(subject.available_commands).to include("IfcConvert")
      end
    end
  end
  ...
  describe "#convert_to_collada" do
    it "uses bundled IfcConvert executable when present" do
      allow(described_class).to receive(:bundled_ifc_converter_path).and_return("/opt/ifc/IfcConvert")
      allow(File).to receive(:file?).with("/opt/ifc/IfcConvert").and_return(true)
      allow(File).to receive(:executable?).with("/opt/ifc/IfcConvert").and_return(true)

      allow(subject).to receive(:convert!).and_yield("target.dae")
      ...
      subject.send(:convert_to_collada, "input.ifc")
    end
  end
end
