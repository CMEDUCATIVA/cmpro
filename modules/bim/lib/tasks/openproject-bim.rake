...
namespace :bim do
  ...
  desc "Ensure bundled IfcConvert binary has executable permissions"
  task ensure_ifcconvert_permissions: :environment do
    bundled_ifc = Rails.root.join("modules", "bim", "vendor", "ifcconvert", "linux-amd64", "IfcConvert")

    unless File.file?(bundled_ifc)
      puts "[bim] bundled IfcConvert not found at #{bundled_ifc}, skipping chmod."
      next
    end

    if File.executable?(bundled_ifc)
      puts "[bim] bundled IfcConvert already executable: #{bundled_ifc}"
      next
    end

    File.chmod(0o755, bundled_ifc)
    puts "[bim] bundled IfcConvert permissions fixed (0755): #{bundled_ifc}"
  rescue StandardError => e
    abort "[bim] failed to ensure IfcConvert permissions: #{e.class}: #{e.message}"
  end
end

begin
  Rake::Task["assets:precompile"].enhance(["bim:ensure_ifcconvert_permissions"])
rescue RuntimeError
  # Ignore when assets tasks are not loaded in this execution context.
end
