      ...
      module Nextcloud
        class FilesQuery
          ...
          def call(auth_strategy:, folder:)
            ...
            files = storage_files(result)
            with_version_labels(files, auth_strategy)
          end
          ...
          def with_version_labels(storage_files_result, auth_strategy)
            return storage_files_result if storage_files_result.failure?

            payload = storage_files_result.result
            return storage_files_result if payload.nil?

            payload_list = payload.is_a?(Array) ? payload : [payload]

            file_ids = payload_list.flat_map(&:files)
                              .map(&:id)
                              .compact
                              .select { |id| id.to_s.match?(/\A\d+\z/) }
                              .uniq

            return storage_files_result if file_ids.empty?

            Rails.logger.info(
              "[storages][nextcloud][files_query] fetching version labels through files_info " \
              "entries=#{file_ids.length}"
            )
            files_info_result = Peripherals::Registry
                                .resolve("#{@storage}.queries.files_info")
                                .call(storage: @storage, auth_strategy:, file_ids:)

            if files_info_result.failure?
              Rails.logger.warn(
                "[storages][nextcloud][files_query] files_info fallback failed for version labels " \
                "error=#{files_info_result.errors&.message || files_info_result.result.inspect}"
              )
              return storage_files_result
            end

            labels_by_id = files_info_result.result.to_h do |file_info|
              [file_info.id.to_s, file_info.version_label]
            end

            enriched_list = payload_list.map do |storage_files|
              enriched_files = storage_files.files.map do |file|
                label = labels_by_id[file.id.to_s]
                file.with(version_label: label.presence || file.version_label.presence || "V")
              end
              storage_files.with(files: enriched_files)
            end

            enriched_payload = payload.is_a?(Array) ? enriched_list : enriched_list.first
            ServiceResult.success(result: enriched_payload)
          end
          ...
          def storage_file(file_element)
            ...
            file_id = id(file_element)
            file_version_label = version_label(file_element).presence || "V"

            Rails.logger.info(
              "[storages][nextcloud][files_query] mapped file version label file_id=#{file_id} " \
              "version_label=#{file_version_label.inspect} location=#{location.inspect}"
            )
            ...
            StorageFile.new(
              ...
              version_label: file_version_label,
              location:,
          ...
          def version_label(element)
            element
              .xpath(".//nc:version-label")
              .map(&:inner_text)
              .reject(&:empty?)
              .first
          end
