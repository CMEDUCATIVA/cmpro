...
module API::V3::FileLinks
  ...
  class FileLinkRepresenter < ::API::Decorators::Single
    ...
    def make_origin_data(model)
      resolved_version_label = resolve_version_label(model)
      Rails.logger.info(
        "[storages][api][file_link_representer] originData version label file_link_id=#{model.id} " \
        "origin_id=#{model.origin_id.inspect} version_label=#{resolved_version_label.inspect}"
      )

      {
        ...
        versionLabel: resolved_version_label
      }
    end

    def resolve_version_label(model)
      return "V" unless model.storage&.provider_type_nextcloud?
      return "V" if model.origin_id.blank?
      return "V" unless current_user&.logged?

      service_result = ::Storages::StorageFileService.call(
        storage: model.storage,
        user: current_user,
        file_id: model.origin_id
      )

      if service_result.success?
        return service_result.result.version_label.presence || "V"
      end

      Rails.logger.warn(
        "[storages][api][file_link_representer] version label lookup failed " \
        "file_link_id=#{model.id} origin_id=#{model.origin_id.inspect} " \
        "error=#{service_result.errors&.message || service_result.result.inspect}"
      )

      "V"
    rescue StandardError => e
      Rails.logger.warn(
        "[storages][api][file_link_representer] version label lookup exception " \
        "file_link_id=#{model.id} origin_id=#{model.origin_id.inspect} " \
